...
export function DailyGraph({ dailyDetails }: dailyRowPorps) {
  ...
  if (dailyDetails && Array.isArray(dailyDetails.hourly_data)) {
    ...
    maxConsToProdIndex = consumptionToProduction.indexOf(
      maxConsumptionToProduction,
    );
    peakPriceHighlight = price.map((p, i) =>
      i === maxConsToProdIndex ? p : null,
    );
  }
  const minPrice = price.length > 0 ? Math.min(...price).toFixed(2) : 0;
  const maxPrice = price.length > 0 ? Math.max(...price).toFixed(2) : 0;
