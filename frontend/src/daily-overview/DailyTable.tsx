...
import { DailyGraph } from "./DailyRow";
...
interface Column {
  id: 'date' | 'avg_daily_price' | 'daily_production' | 'daily_consumption' | 'negative_streak';
  label: string;
  minWidth?: number;
  align?: 'right';
...
export function DailyTable({dailyData}:DailyTableProps){
  ...
  const handleChangeRowsPerPage = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const details = dailyData[clickedDay] || []
    return(
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div>
           <div key={clickedDay}><DailyGraph dailyDetails={details}/></div>
        </div>
            ...
            {Object.entries(dailyData)
              ...
              .map(([dateKey,row]) => {
                return (
                  ...
                  key={dateFormatForUI(dateKey)} sx={{
                    backgroundColor: clickedDay === dateKey ? 'rgba(25, 118, 210, 0.12)' : 'inherit',
                    '& .MuiTableCell-root': {
                    color: clickedDay === dateKey ? 'primary.main' : 'inherit',
                    fontWeight: clickedDay === dateKey ? 'bold' : 'normal',
                    },
                    '&:hover': {
                    backgroundColor: clickedDay === dateKey ? 'rgba(25, 118, 210, 0.2)' : undefined,
                    }}}>
                    {columns.map((column) => {
